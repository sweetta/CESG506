...
# Problem 6-3 Study the snap-through behavior of a shallow arch
...
# Convergence criteria and arc-length initialization parameters
max_iteration = 20
tol = 1e-9
a = 0.0
delta_s_Factor = 10
...
umidIdx = round(nEle/2)*dim + 1     # keeping track of vertical displacement of middle node
uq1Idx = round(nEle/4)*dim + 1      # keeping track of vertical displacement of 1/4 point node
uq3Idx = round(3*nEle/4)*dim + 1    # keeping track of vertical displacement of 3/4 point node

# Construct system based off of number of elements and user's elevation function
Lx = W / nEle
...
# Initial Steps to find delta_s^2
...
# Arc-length method loop now that delta_s is determined
flag = 0
for i in range(500):
...
plt.ylabel('$\gamma$')
plt.title('$\gamma$ vs Disp for {} Element Mesh'.format(nEle))

plt.legend(loc='best', ncol=1, framealpha=1)
plt.axhline(y=0, color='black')
plt.grid(True)
plt.savefig("CESG506/Code/Assignments/Assignment 6/Problem6-3/Prob6-3_GammaVsDisp_{}Ele".format(nEle))
plt.show()
