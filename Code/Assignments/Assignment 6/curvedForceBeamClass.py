...
class curvedForceBeamClass(object):
    """
    class: curvedForceBeamClass
    ...
    def setDisp(self, U1, U2):
        ...
        self.F = 0
        XW = gaussQuad(2, self.Lx)
        for xw in XW:
            du0 = np.dot(dNu(xw[0], self.Lx), self.q)
            dv0 = np.dot(dNv(xw[0], self.Lx), self.q)
            self.F += xw[1]*self.EA*(du0+dv0*self.dh+0.5*dv0*dv0)/self.Lx
        ...
        for xw in XW:
            ...
            # kf = self.EA*np.dot(D, D)/self.Lx
            km = self.EI*np.outer(Bp, Bp)
            kg = self.F*np.outer(dNv(xw[0], self.Lx), dNv(xw[0], self.Lx))
            D += xw[1]*Be
            self.ke += xw[1]*(km+kg)
            self.Fint += xw[1]*(self.F*Be + M*Bp)
        self.ke += self.EA*np.outer(D, D)/self.Lx
    # def setDisp(self, U1, U2):
    #     self.ke = np.zeros((6, 6))
    #     self.Fint = np.zeros(6)
    #     self.U1 = U1
