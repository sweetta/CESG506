...
Pref = 119.5    # Reference Load [kN]

# Convergence criteria
a = 0.0
max_iteration = 50
tol = 1e-10     # np.linalg.norm([R],[g]) <= tol
...
for i in range(105):
    ...
    Uu.append(u_guess[46])
    Uv.append(u_guess[47])
...
for i in range(24):
    plt.plot(u_list[2*i] + X[i][0], u_list[2*i+1] + X[i][1], label="$U_u$", marker='.')
# plt.plot(u_list[44] + X[22][0], u_list[45] + X[22][1], label="$U_u$", marker='.')
# plt.plot(u_list[46] + X[23][0], u_list[47] + X[23][1], label="$U_u$", marker='.')
